...
#
# Two flags are appended here:
#   -pthread          enables the wasm `atomics`/`bulk-memory` features. Without
#                     them wasm-ld rejects `--shared-memory` at the final
#                     wasm_threads link (e.g. avro -> jansson's
#                     hashtable_seed.c.o).
#   -fwasm-exceptions builds every dependency with *native* wasm exception
#                     handling, matching the engine (also -fwasm-exceptions).
#                     Without it deps compile with legacy emscripten EH and emit
#                     invoke_*/__resumeException/getTempRet0 trampolines the
#                     native-EH engine never provides, so a dep's try/catch is
#                     inert and the exception escapes uncaught (e.g. PROJ's
#                     guarded std::stoi on the OGC:CRS84 code throws a fatal
...
foreach(_lang C CXX)
  string(APPEND CMAKE_${_lang}_FLAGS " -pthread -fwasm-exceptions")
  string(APPEND CMAKE_${_lang}_FLAGS_INIT " -pthread -fwasm-exceptions")
endforeach()
foreach(_link EXE SHARED MODULE)
  string(APPEND CMAKE_${_link}_LINKER_FLAGS " -pthread -fwasm-exceptions")
  string(APPEND CMAKE_${_link}_LINKER_FLAGS_INIT " -pthread -fwasm-exceptions")
endforeach()
